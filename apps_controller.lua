...
module("luci.controller.apps.apps_controller", package.seeall)
...
function index()
    ...
    entry({"apps", "list"}, cbi("apps/apps_cbi")).dependent=false
    entry({"apps", "add"}, template("apps/add_view")).dependent=false
...
for _, r in pairs(apps) do
   if r.type then
	 ...
	 luci.template.render("apps/apps_view", {categories=categories})
end
